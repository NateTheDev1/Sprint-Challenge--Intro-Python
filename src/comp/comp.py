...
c = [person.name for person in humans if any(person.name.startswith(item) for item in ['C', 'D', 'E', 'F', 'G'])]
print(c)
...
f = [tuple([person.name, person.age]) for person in humans if any(person.age == item for item in [27, 28, 29, 30, 31, 32])]
print(f)
